...
Решим задачу с помощью геометрического метода. Для этого составим \hyperref[sec:duality]{двойственную} ей задачу.
...
Неравенство $y_1 \le 3 строже чем y_1 \le 4$, поэтому второе можно не учитывать, получаем:
...
Нарисуем прямую $20y_1 + 12y_2 = 30 \; (c=36)$

\begin{figure}[H]
	\centering
		\begin{tikzpicture}
		\begin{axis}[axis lines=middle, xlabel={$y_1$}, ylabel={$y_2$}, grid=major, width=0.7\textwidth, xmin=-1, xmax=6, ymin=-2, ymax=6]	
			\addplot[domain=-1:6, samples=2, green, thick]{-20/12 * x + 3};
...
\begin{align*}
	&\begin{cases}
		2x_1 + x_2 = 20, \\
		2x_2 = 12; \quad \Big| \quad \cdot 0.5
	\end{cases} \\
	&\begin{cases}
		2x_1 + x_2 = 20, \\
		x_2 = 6;
...
\begin{enumerate}
	...
	\item[\fbox{Шаг 1}] Видно, что коэффициент перед $x_2$ в выражении $x_0$ отрицателен.
	...
	\begin{linear}{rrrrrrrrrrr}
		& \frac{2}{3}x_2 & = & 4 & - & x_3 & - & \frac{2}{3}x_5 & + & \frac{1}{3}y_2, \\
		& x_2 & = & 6 & - & \frac{3}{2}x_3 & - & x_5 & + & \frac{1}{2}y_2. \\
	\end{linear}
	\]
	...
	\begin{linear}{rrrrrrrrrrrrrrrrrrr}
		...
		&& -\frac{1}{2}&(&6 && & - &\frac{3}{2}x_3&&& - &x_5&& & & + &\frac{1}{2}y_2) & = \\
