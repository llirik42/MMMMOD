...
Решим данную задачу с помощью геометрического метода.

Изобразим на плоскости прямые, соответствующие ограничениям, точку их пересечения $O$ и множество допустимых решений:
...
Нарисуем прямую $4x_1+ 5x_2 = 10$ $(c=10)$ и посмотрим как она будет лежать.
...
Параллельно передвигая эту прямую вдось оси $x_2$ можно заметить, что максимальное $c = 20$, когда прямая пересекает область в точке $(5, 0)$.
...
Для решения данной задачи составим двойственную ей задачу.
...
Изобразим на плоскости прямые, соответствующие ограничениям и множество допустимых решений:
...
Областью, задаваемыми неравенствами оказался треугольник $ABC$

Нарисуем прямую $19y_1 + 57y_2 = 150 \; (c=150)$
...
Параллельно передвигая эту прямую вдось оси $y_2$ можно заметить, что минимальное $c$ достигается когда прямая проходит через точку $A$. Найдем её координаты, пользуясь тем, что эта точка - пересечение прямых $y_1 + 4y_2 = 24$ и $5y_1 + y_2 = 25$.
...
Данный результат еще не дает ответ: необходимо найти $x_1 \cdots x_5$. Для этого воспользуюемся соотношениями дополняющей нежесткости.

\bigskip

\textbf{Первое соотношение дополняющей нежесткости}
...
Подставляем найденные $y_1, y_2$ и получаем
...
\begin{note}
	Все $x_j = 0$, кроме тех $j$, номера прямых которых в пересечении дали точку $A$, это были прямые $2, 5$ (по соответствующим позициям в системе неравеств на $y_1, y_2$)  
\end{note}

\bigskip

\textbf{Второе соотношение дополняющей нежесткости}
...
Применим результат, который получился при перении первого соотношения дополняющей нежесткости (удалим все $x_j = 0$).
...
Найдем значение целевой функции при $\overline{X} = (0, 14, 0, 0, 1)$
\[
-10 \cdot 0 + 24 \cdot 14 + 20 \cdot 0 + 20 \cdot 0 + 25 \cdot 1 = 361 = c'
\]

Всё совпало. Ответ: $(0, 14, 0, 0, 1)$
...
Параллельно передвигая эту прямую вдось оси $y_2$ можно заметить, что минимальное $c$ достигается когда прямая проходит через точку $C$. Найдем её координаты, пользуясь тем, что эта точка - пересечение прямых $y_1 + 4y_2 = 24$ и $3y_1 + 2y_2 = 20$.
...
Зная что точка $C$ это пересечение прямх с номерами $2, 4$ получаем сразу:
...
Напишем его сразу используя результаты первого соотношения дополняющей нежесткости.
...
Найдем значение целевой функции при $\overline{X} = (0, 13.3, 0, 1.9)$
\[
-10 \cdot 0 + 24 \cdot 13.3 + 20 \cdot 0 + 20 \cdot 1.9 = 357.2 = c'
\]

Всё совпало. Ответ: $(0, 13.3, 0, 1.9)$
