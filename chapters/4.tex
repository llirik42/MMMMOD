...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\begin{axis}[axis lines=middle, xlabel={$x_1$}, ylabel={$x_2$}, grid=major, width=0.8\textwidth, xmin=-1, xmax=6, ymin=-2, ymax=5]
...
То что заштрихованная на рисунке выше область являются областью допустимых значений следует из следующего:
\begin{itemize}[nosep]
	\item $4x_1 = 3x_2 \le 12$, значит допустимые решения должны лежать не выше прямой $4x_1 + 3x_2 = 12$;
	
	\item $2x_1 + 5x_2 \le 10$, значит допустимые решения должны лежать не выше прямой $2x_1 + 5x_2 = 10$;
	
	\item в задаче линейного программирования $x_j \ge 0, j = 1 \dots n$, поэтому $x_1, x_2 \ge 0$.
\end{itemize}

То есть нас интересуют точки плоскости, находящиеся в первой четверти и лежащие не выше прямых, соответствующих ограничениям.

Целевой функции соответствует прямая $4x_1 + 5x_2 = c$, где $c$ --- неизвестное, которое нужно найти. Для примера нарисуем прямые при $c = 10$ и $c = 20$ (рис. 4.2).
...
Ясно, что если брать $c > 20$, то прямая целевой функции будет отдаляться от области допустимых решений, поэтому нужно брать $c < 20$. Задача нахождения максимума означает, что нам нужно найти максимальное число $c'$ такое, что прямая $4x_1 + 5x_2 = c'$ пересекает множество допустимых решений.

Ясно, что $c'$ соответствует прямой, которая пересекает точку $O$ (рис. 4.3).
...
Таким образом, при исходных ограничениях

\[
\max_{(x_1, x_2)} \big(4x_1 + 5x_2\big) = \frac{100}{7}.
\]
