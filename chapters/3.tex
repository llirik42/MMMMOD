...
\fact[свойство разрезов]\label{fact:sections_property}
...
Если для потока $x = (x_ik)$ существует такой разрез $(S, \bar{S})$, что
\[
v(x) = B(S, \bar{S}),
\]
...
Доказательство основано на \hyperref[fact:sections_property]{свойствах разреза}.

\subsection{Алгоритм Форда-Фалкерсона}

\algorithm[Алгоритм Форда-Фалкерсона поиска максимального потока]\label{alg:ford_Fulkerson}

Алгоритм Форда-Фалкерсона позволяет находить максимальный поток в сети. Идея алгоритма следующая:
\begin{enumerate}[nosep]
	\item Изначально по сети пускаем поток нулевой величины.
	
	\item На каждой итерации пытаемся найти путь из вершины $1$ в вершину $n$, в котором пропускные способности дуг позволили бы увеличить поток (\textbf{построение пути}). Если такой путь найден, то увеличиваем поток (\textbf{изменение потока}) и повторяем этот шаг, а если нет, то завершаем алгоритм.
\end{enumerate}

\definition

Каждая вершина является либо \definitionfont{не помеченной}, либо \definitionfont{помеченной и просмотренной}, лбо \definitionfont{помеченной и не просмотренной}.

\definition

\definitionfont{Пометкой} вершины $i$ будем называть пару $\big(\pm k, \varepsilon(i)\big)$.

$k$ --- номер помеченной вершины, из которой мы помечаем вершину $i$. Если мы помечаем по прямой дуге, то ставим $+k$, а если по обратной, то ставим $-k$.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\vfil
	\caption{Прямые и обратные дуги}
\end{figure}

$\varepsilon(i)$ --- максимальная величина потока, который может пройти от вершины $1$ до вершины $i$ через вершину $k$.

\definition

Вершина является \definitionfont{помеченной}, если у неё ест пометка.

\definition

Вершина является \definitionfont{просмотренной}, если она и все смежные с ней вершины помечены.

Алгоритм

\begin{enumerate}
	\item Вершина $1$ помечена.
	
	\item Пусть вершина $k$ имеет пометку $\big(\dots, \varepsilon(k)\big)$, а $i$ нет, тогда помечаем вершину $i$ из вершины $k$:
	
	\begin{itemize}[nosep]
		\item по прямой дуге (увеличиваем поток): если $b_ki - x_ki > 0$, то
		\[
		\varepsilon(i) = \min\big\{\varepsilon(k), b_ki - x_ki\big\}
		\]
		
		\item по обратной дуге (уменьшаем поток): если $x_ki > 0$, то
		\[
		\varepsilon(i) = \min\big\{\varepsilon(k), x_ki\big\}
		\]
		
	\item Если вершина $n$ оказывается помеченной, то строим путь от вершины $n$ до вершины $1$, двигаясь по первым компонентам пометок всех вершин. Пусть $\varepsilon(n) = \varepsilon$, меняем поток через дуги пути следующим образом:
	...
	\item Если пометить вершину $n$ не удаётся, то завершаем алгоритм и строим разрез. Множество $S$ будет состоять из вершин, которые удалось пометить на данной итерации, а в $\bar{S}$ --- которые не удалось. Заметим, что разрез тут будет корректным: вершина $1 \in S$, а вершина $n \in \bar{S}$. Разрез $(S, \bar{S})$ будем разрезом, на котором достигается равенство $v(x) = B(S, \bar{S})$. Величину $v(x)$ можно найти либо как сумму всех $\varepsilon{n}$ по всем итерациям, либо через <<балансы>> вершин $1$ или $n$.
...
После изменения потока (такого-то шага) поток останется потоком.
...
Во всех случаях после выполнения итерации баланс произвольной вершины $i$ не поменялся, значит поток $x$ после изменения остался потоком.
