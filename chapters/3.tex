...
\begin{itemize}[nosep]
	...
	\[
	\varepsilon(i) = \min\big\{\varepsilon(k), x_ki\big\}
	\]
\end{itemize}
...
В каждом случае <<балансы>> произвольной вершины $i$ до и после изменения потока совпадают (таб. 3.1, 3.2, 3.3, 3.4), значит ограничение (1) из определения потока продолжает выполняться, значит после каждого изменения на каждой итерации имеем корректный поток.
...
Найти максимальный поток в сети, изображённой на рисунке (возле каждой изображена её пропускная способность $b_{ik}$).
