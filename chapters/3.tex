...
\begin{definition}
	Поток $x = (x_ik)$ будем называть \definitionfont{максимальным}, если величина $v(x)$ максимальна.
\end{definition}

\fact[признак максимальности потока]\label{fact:max_flow_sign}

Если для потока $x = (x_{ik})$ существует такой разрез $(S, \bar{S})$, что
\[
v(x) = B(S, \bar{S}),
\]

то поток $x$ является максимальным.
...
\begin{enumerate}[nosep]
	...
	\item Если пометить вершину $n$ не удаётся, то завершаем алгоритм и строим разрез $(S, \bar{S})$, на котором будет достигаться $v(x) = B(S, \bar{S})$. Это будет означать, что полученный поток действительный максимальный (см. \hyperref[fact:max_flow_sign]{Признак максимальности потока}). Разрез определяется следующим образом:
