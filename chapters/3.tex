...
Необходимо вычислить макисмальный поток в сети.
...
Будем решать задачу с помощью алгоритма Форда-Фалкерсона.
...
Колонки будут соответствовать итерациям алгоритма Форда-Фалкерсона. Таблица предзаполнена значениями, чтобы начать первую итерацию (нулевая итерация соответствуюет установлению всех потоков в 0).

Таблица имеет две секции, в первой из них находятся пары $(k, \varepsilon(i))$ - пометки для сооветствующих вершин.

\remark{Поскольку вершина 1 начальная, то все её пометки будут иметь значение $(-, \infty)$.}

\bigskip

Во второй секции для каждой дуги в итерации записана пара $(b, x)$ --- пропускная способность и поток для текущей итерации.

\bigskip

\textbf{Заполнение таблицы}
\begin{itemize}[nosep]
	\item[\fbox{Шаг 1}] Используя таблицу ставим метки вершинам, пока не пометим вершину $n = 5$.
	
	\bigskip
	
	\begin{enumerate}[nosep]
		\item[\underline{$i=2$}:] можно пометить из вершины с номером 1, $\varepsilon(+1) = \min(\infty, 3-0)=3$ $\Rightarrow$ метка: $(+1, 3)$
		\item[\underline{$i=3$}:] можно пометить из вершины с номером 1, $\varepsilon(+1) = \min(0, 5-0)=5$ $\Rightarrow$ метка: $(+1, 5)$
		\item[\underline{$i=4$}:] можно пометить из вершины с номером 2, $\varepsilon(+2) = \min(3, 6-0)=3$ $\Rightarrow$ метка: $(+2, 3)$
		\item[\underline{$i=5$}:] можно пометить из вершины с номером 4, $\varepsilon(+4) = \min(3, 4-0)=3$ $\Rightarrow$ метка: $(+4, 3)$
	\end{enumerate}
	...
	Нам удалось пометить вершину 5 и $\varepsilon$ метки вершины 5 равна 3.
	
	\bigskip
	
	Используя метки восстанавливаем путь с максимальным потоком от вершины 5 до вершины 1 (первое значение метки --- это та вершина, откуда в эту вершину пришли, а знак --- направление дуги). Над дугами записываем значение на которое нужно будет увеличить значение потока соотвтетсующей дуги ($\varepsilon$ или -$\varepsilon$) в зависимости от направления.
	...
	Изменяем поток соответствующих дуг, записываем результат в таблицу (в следующую итерацию).
	...
	\item[\fbox{Шаг 2}] Используя таблицу ставим метки вершинам, пока не пометим вершину $n = 5$.
	
	\bigskip
	
	\begin{enumerate}[nosep]
		\item[\underline{$i=2$}:] нельзя пометить из вершнины 1 (для дуги 1 не выполняется условие $b_{1,2} - x_{1,2} > 0$).
		\item[\underline{$i=3$}:] можно пометить из вершины с номером 1, $\varepsilon(+1) = \min(\infty, 5-0)=5$ $\Rightarrow$ метка: $(+1, 5)$
		\item[\underline{$i=4$}:] можно пометить из вершины с номером 3, $\varepsilon(+3) = \min(5, 2-0)=2$ $\Rightarrow$ метка: $(+3, 2)$
		\item[\underline{$i=5$}:] можно пометить из вершины с номером 3, $\varepsilon(+3) = \min(5, 2-0)=2$ $\Rightarrow$ метка: $(+3, 2)$
	\end{enumerate}
	...
	Нам удалось пометить вершину 5 и $\varepsilon$ метки вершины 5 равна 2.
	...
	\item[\fbox{Шаг 3}] Используя таблицу ставим метки вершинам, пока не пометим вершину $n = 5$.
	
	\bigskip
	
	\begin{enumerate}[nosep]
		\item[\underline{$i=2$}:] нельзя пометить из вершины 1.
		\item[\underline{$i=3$}:] можно пометить из вершины с номером 1, $\varepsilon(+1) = \min(\infty, 5-2)=3$ $\Rightarrow$ метка: $(+1, 3)$
		\item[\underline{$i=4$}:] можно пометить из вершины с номером 3, $\varepsilon(+3) = \min(3, 2-0)=2$ $\Rightarrow$ метка: $(+3, 2)$
		\item[\underline{$i=5$}:] можно пометить из вершины с номером 4, $\varepsilon(+4) = \min(2, 4-3)=1$ $\Rightarrow$ метка: $(+4, 1)$
	\end{enumerate}
	...
	Нам удалось пометить вершину 5 и $\varepsilon$ метки вершины 5 равна 1.
	...
	\item[\fbox{Шаг 4}] Используя таблицу ставим метки вершинам, пока не пометим вершину $n = 5$.
	
	\bigskip
	
	\begin{enumerate}[nosep]
		\item[\underline{$i=2$}:] нельзя пометить из вершины 1.
		\item[\underline{$i=3$}:] можно пометить из вершины с номером 1, $\varepsilon(+1) = \min(\infty, 5-3)=2$ $\Rightarrow$ метка: $(+1, 2)$
		\item[\underline{$i=4$}:] можно пометить из вершины с номером 3, $\varepsilon(+3) = \min(2, 2-1)=1$ $\Rightarrow$ метка: $(+3, 1)$
		\item[\underline{$i=2$}:] можно пометить из вершины с номером 4, $\varepsilon(-4) = \min(1, 3)=1$ $\Rightarrow$ метка: $(-4, 1)$
		\item[\underline{$i=5$}:] нельзя пометить ни из каких вершин (3, 4).
	\end{enumerate}
	...
	Нам не удалось пометить вершину 5. Итерации завершены.
	
\end{itemize}

\bigskip

Используя готовую таблицу, можно построить разбиение $S$ и $\bar{S}$. В $S$ помеченные и проверенные вершины, в $\bar{S}$ все остальные. Получаем:
\[S = \big\{1, 2, 3, 4\big\}, \bar{S} = \big\{5\big\}\]

Вычислим разрез: $B(S, \bar{S}) = b_{3,5} + b_{4,5} = 2 + 4 = 6$

Максимальный поток: $v = x_{1,3} + x_{1,2} = 3 + 3 = 6$ (Значения для $x_{1,3}, x_{1,2}$ берутся из таблицы: смотрится значение потока в последней итерации для соответствующих дуг).