...
\begin{itemize}[nosep]
	...
	\item $C = \big\{(i, j) \; \big| \; i,j \in J\big\}$ --- частичный порядок, работа $j$ не может начаться раньше окончания работы $i$.
\end{itemize}
...
Изобразим сетевую модель проекта из \hyperref[ex:office_project]{Примера 8}.
...
\fact[резерв времени работы]\label{fact:max_delta_j}
...
Можно заметить, что максимальный (критический) путь не содержит дугу $3$. Для примера вычислим резерв времени выполнения работы $3$, то есть на сколько максимально можно увеличить время этой работы, чтобы общее время не увеличилось. По утверждению \hyperref[fact:max_delta_j]{Резерв времени работы}
...
\begin{itemize}[nosep]
	...
	\item $f(5)$ берётся из заполненной таблицы;
...
тогда $x = (x_{ik})$ будем называть \definitionfont{потоком в сети} с \definitionfont{величиной потока} $v(x)$, если выполняются следующие ограничения
\[
...
Ограничение (1) будем называть \definitionfont{уравнением баланса} вершин.

\definition

Пусть $l \in I$, тогда величину
\[
\sum_{k:\;(l, k) \in J} x_{lk} - \sum_{i: \; (i, l) \in J} x_{il}
\]

будем называть \definitionfont{балансом} для вершины $l$.
...
\fact[свойство разрезов]\label{fact:section_property}
...
\begin{itemize}[nosep]
	...
	Рассмотрим некоторую дугу $(i, k) \in E(l)$, на рисунке 3.11 изображены возможные случаи взаимного расположения её вершин.
...
\begin{definition}
	Поток $x = (x_{ik})$ будем называть \definitionfont{максимальным}, если величина $v(x)$ максимальна.
\end{definition}

\fact[критерий максимальности потока]\label{fact:max_flow_criterium}

Поток $x = (x_{ik})$ является максимальным $\Longleftrightarrow$ существует такой разрез $(S, \bar{S})$, что
\[
v(x) = B(S, \bar{S}).
\]
...
Доказательство основано на \hyperref[fact:section_property]{Свойстве разреза}.

\section{Алгоритм Форда-Фалкерсона поиска максимального потока}\label{alg:ford_Fulkerson}
...
\begin{enumerate}[nosep]
	...
	\item Изначально пускаем по сети поток нулевой величины.
	
	\item На каждой итерации пытаемся найти путь из вершины $1$ в вершину $n$, в котором пропускные способности дуг позволили бы увеличить поток. Если такой путь найден, то увеличиваем поток и повторяем этот шаг. Если путь не найден, то завершаем алгоритм.
\end{enumerate}

По ходу алгоритма будем помечать определённым образом вершины сети. 

\definition

Фраза \definitionfont{пометить вершину $i$ из вершины $k$} будет означать, что вершине $i$ присвоена некоторая пометка --- пара $\big(\pm k, \varepsilon(i)\big)$, где

\begin{itemize}[nosep]
	\item $k$ --- номер помеченной вершины; если дуга между вершинами прямая (рис. 3.12), то пишем $+k$, а если обратная (рис. 3.13), то пишем $-k$;
	
	\item $\varepsilon(i)$ --- максимальная величина потока, который может пройти от вершины $1$ до вершины $i$ через вершину $k$.
\end{itemize}

\begin{figure}[H]
	\centering	
	\begin{tikzpicture}
		\node[vertex, label=left:{$k$}] (k) at (-0.8,0) {};
		\node[vertex, label=right:{$i$}] (i) at (0.8,0) {};
		\draw[->, edge] (k) to[bend left=20] (i);
		\node at (0,0.5) {$+k$};
	\end{tikzpicture}
	\caption{Прямая дуга}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\node[vertex, label=left:{$i$}] (i) at (-0.8,0) {};
		\node[vertex, label=right:{$k$}] (k) at (0.8,0) {};
		\draw[->, edge] (i) to[bend left=20] (k);
		\node at (0,0.5) {$-k$};
	\end{tikzpicture}
	\caption{Обратная дуга}
\end{figure}

\definition

Вершина является \definitionfont{помеченной}, если у неё есть пометка.

\definition

Вершина является \definitionfont{просмотренной}, если она помеченная и все смежные с ней вершины либо не могут быть помечены из неё, либо уже помечены.

\definition[правила помечаний]

Пусть вершина $k$ имеет пометку $\big(\dots, \varepsilon(k)\big)$, а вершина $i$ не помеченная, но смежная с $k$, тогда вершину $i$ можно пометить из вершины $k$

\begin{itemize}[nosep]
	\item по прямой дуге если $b_{ki} - x_{ki} > 0$, при этом
	\[
	\varepsilon(i) = \min\big\{\varepsilon(k), b_{ki} - x_{ki}\big\}
	\]
	
	\item по обратной дуге если $x_{ki} > 0$, при этом
	\[
	\varepsilon(i) = \min\big\{\varepsilon(k), x_{ki}\big\}
	\]
\end{itemize}

Во всех остальных случаях пометить вершину $i$ из вершины $k$ нельзя.
...
\begin{enumerate}[nosep]
	...
	\item Присваиваем вершине $1$ пометку $(-, \infty)$.
	
	\item Пусть вершина $i$ не помеченная, но её можно пометить из вершины $k$. Помечаем вершину $i$ из $k$ по правилам.
	
	\item Если вершина $n$ оказывается помеченной, то строим путь от вершины $n$ до вершины $1$, двигаясь по первым компонентам пометок всех вершин (рис 3.14). Пусть $\varepsilon(n) = \varepsilon$, меняем поток через дуги пути следующим образом:
	
	\begin{itemize}[nosep]
		\item если дуга в пути прямая, то увеличиваем поток через неё на $\varepsilon$,
		
		\item если дуга в пути обратная, то уменьшаем поток через неё на $\varepsilon$;
	\end{itemize}
		
	После изменения потока через дуги переходим на следующую итерацию (возвращаемся к шагу 1).
		
	\item Если пометить вершину $n$ не удаётся, то завершаем алгоритм и строим разрез $(S, \bar{S})$, на котором будет достигаться $v(x) = B(S, \bar{S})$. Это будет означать, что полученный поток действительный максимальный (см. \hyperref[fact:max_flow_criterium]{Критерий максимальности потока}). Разрез определяется следующим образом:
	
	\begin{itemize}[nosep]
		\item $S$ --- множество вершин, которые удалось пометить на последней итерации; $1 \in S$;
		
		\item $\bar{S}$ --- множество вершин, которые не удалось пометить на последней итерации, $n \in \bar{S}$.
	\end{itemize}
	
	Величину $v(x)$ можно найти 3 способами:
	
	\begin{itemize}[nosep]
		\item через <<баланс>> вершины $1$
		\[
		v(x) = \sum_{k:\;(1, k) \in J} x_{1k}
		\]
		
		\item через <<баланс>> вершины $n$,
		\[
		v(x) = \sum_{i: \; (i, n) \in J} x_{in}
		\]
		
		\item как сумму $\varepsilon$ по всем итерациям. 
	\end{itemize}
\end{enumerate}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[vertex] (1) at (0, 0) {$1$};
		\node[vertex] (i1) at (1, 0) {$i_1$};
		\node[vertex] (l) at (2, 0) {$l$};
		\node[vertex] (i) at (3, 0) {$i$};
		\node[vertex] (k) at (4, 0) {$k$};
		\node[vertex] (n) at (5, 0) {$n$};
		
		\node at (1,0.4) {$(+1, \dots)$};
		\node at (1.5,0) {$\dots$};
		\node at (2,0.4) {$(\dots, \dots)$};
		\node at (3,0.4) {$(-l, \dots)$};
		\node at (4,0.4) {$(+i, \dots)$};
		\node at (5,0.4) {$(+k, \varepsilon)$};
		\draw[->, edge] (1) -- node {} (i1);
		\draw[->, edge] (i) -- node {} (l);
		\draw[->, edge] (i) -- node {} (k);
		\draw[->, edge] (k) -- node {} (n);
...
\fact[корректность изменения потока]

После изменении потока через дуги после итерации свойство <<баланса>> вершин продолжает сохраняться.

\prooof

Рассмотрим всевозможные случаи взаимного расположения прямых и обратных дуг вершины $i$ (рис. 3.15).
...
\begin{figure}[H]
	...
	\begin{subfigure}{\linewidth}
		...
		\begin{tikzpicture}[
			...
			\node[vertex] (1) at (0, 0) {$\dots$};
			\node[vertex] (2) at (1, 0) {$i$};
			\node[vertex] (3) at (2, 0) {$\dots$};
			
			\draw[->, edge] (1) -- node[below] {$x$} node[above] {$+\varepsilon$} (2);
			\draw[->, edge] (2) -- node[below] {$y$} node[above] {$+\varepsilon$} (3);
		\end{tikzpicture}
		\caption{Случай 1}
	\end{subfigure}
	\vfil
	\begin{subfigure}{\linewidth}
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {$x$} node[above] {$+\varepsilon$} (2);
			\draw[->, edge] (3) -- node[below] {$y$} node[above] {$-\varepsilon$} (2);
		\end{tikzpicture}
		\caption{Случай 2}
	\end{subfigure}
	\vfil
	\begin{subfigure}{\linewidth}
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (2) -- node[below] {$x$} node[above] {$-\varepsilon$} (1);
			\draw[->, edge] (3) -- node[below] {$y$} node[above] {$-\varepsilon$} (2);
		\end{tikzpicture}
		\caption{Случай 3}
	\end{subfigure}
	...
	\begin{subfigure}{\linewidth}
		...
		\caption{Случай 4}
	\end{subfigure}
	\caption{Всевозможные случаи взаимного расположения дуг вершины $i$}
\end{figure}

В каждом случае <<балансы>> произвольной вершины $i$ до и после изменения потока совпадают (таб. 3.1, 3.2, 3.3, 3.4), значит ограничение (1) из определения потока продолжает выполняться, поэтому после каждого изменения на каждой итерации имеем корректный поток.
...
\begin{table}[H]
	...
	\begin{tabular}{ | c | c | } 
		\hline
		Было & Стало \\\hline
		$-x + y$ & $-(x + \varepsilon) + (y + \varepsilon) = -x + y$ \\\hline
	\end{tabular}
	\caption{Выражение <<баланса>> для случая 1}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ | c | c | } 
		...
		$-x - y$ & $-(x + \varepsilon) - (y - \varepsilon) = -x - y$ \\\hline
	\end{tabular}
	\caption{Выражение <<баланса>> для случая 2}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ | c | c | } 
		...
		$x - y$ & $(x - \varepsilon) - (y - \varepsilon) = x - y$ \\\hline
	\end{tabular}
	\caption{Выражение <<баланса>> для случая 3}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ | c | c | } 
		...
		$x + y$ & $(x - \varepsilon) + (y + \varepsilon) = x + y$ \\\hline
	\end{tabular}
	\caption{Выражение <<баланса>> для случая 4}
\end{table}

\section{Практика вычисления максимального потока}

\problem

Найти максимальный поток в сети, изображённой на рисунке (возле каждой дуги изображена её пропускная способность $b_{ik}$).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\draw[->, edge] (1) -- node[above=2mm, left=1mm] {\squared{1}} node[below=1mm, right=1mm] {$3$} (2);
		\draw[->, edge] (1) -- node[below left] {\squared{3}} node[above right] {$5$} (3);
		\draw[->, edge] (2) -- node[above] {\squared{2}} node[below] {$6$} (4);
		\draw[->, edge] (3) -- node[below right] {\squared{5}} node[above left] {$2$} (4);
		\draw[->, edge] (3) -- node[below] {\squared{4}} node[above] {$2$} (5);
		\draw[->, edge] (4) -- node[right] {\squared{6}} node[left] {$4$} (5);	
	\end{tikzpicture}
	\caption{Сеть}
\end{figure}
...
Будем решать задачу с помощью \hyperref[alg:ford_Fulkerson]{Алгоритма Форда-Фалкерсона}. Для этого будем использовать следующую таблицу:
	
\begin{table}[H]
	\centering
	\begin{tabular}{ | c | c | } \hline
		\diagbox{Вершина}{Итерация} & 1 \\ \hline
		1 & $(-, \infty)$  \\ \hline
		2 &  \\ \hline
		3 &  \\ \hline
		4 &  \\ \hline
...
Таблица разделена на 2 секции:
\begin{enumerate}[nosep]
	\item В первой секции напротив вершин будут находиться пары $\big(\pm k, \varepsilon(i))$ --- пометки вершин.
	
	\item Во второй секции для каждой дуги $j=\vec{ik}$ будет находиться пара $(b_{ik}, {x_ik})$, где
	
	\begin{itemize}[nosep]
		\item $b_{ik}$ --- пропускная способность дуги,
		
		\item $x_{ik}$ --- величина потока, который проходит через дугу.
	\end{itemize}
\end{enumerate}

Столбцы будут соответствовать итерациям алгоритма. Таблица предзаполнена значениями, чтобы начать первую итерацию (нулевая итерация соответствует установлению величины исходного потока в $0$).

\begin{note}
	По алгоритму начальная вершина $1$ всегда помечена, но поскольку её пометка не несёт смысла, то напротив первой вершины будем писать $(-, \infty)$.
\end{note}
...
На каждой итерации будем пытаться найти путь, который позволит увеличить поток из первой в последнюю вершину. То есть на каждом шаге будем помечать вершины, пока не пометим вершину $n = 5$.
...
\begin{itemize}[nosep]
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=2$ из вершины $k=1$ по прямой дуге $j=1$:
		\[
		\varepsilon(2) = \min(\underbrace{\infty}_{\varepsilon(1)}, \underbrace{3-0}_{b_1 - x_1})=3 \quad \Longrightarrow \quad \underbrace{(+1, 3)}_{\text{пометка для $i=2$}}
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=1$ по прямой дуге $j=3$:
		\[
		\varepsilon(3) = \min(\underbrace{\infty}_{\varepsilon(1)}, \underbrace{5-0}_{b_3-x_3})=5 \quad \Longrightarrow \quad \underbrace{(+1, 5)}_{\text{пометка для $i=3$}}
		\]
		 
		\item Помечаем вершину $i=4$ из вершины $k=2$ по прямой дуге $j=2$:
		\[
		\varepsilon(4) = \min(\underbrace{3}_{\varepsilon(2)}, \underbrace{6-0}_{b_2-x_2})=3 \quad \Longrightarrow \quad \underbrace{(+2, 3)}_{\text{пометка для $i=4$}}
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=4$ по прямой дуге $j=6$:
		\[
		\varepsilon(5) = \min(\underbrace{3}_{\varepsilon(4)}, \underbrace{4-0}_{b_6-x_6})=3 \quad \Longrightarrow \quad \underbrace{(+4, 3)}_{\text{пометка для $i=5$}}
		\]
	\end{enumerate}
	...
	Нам удалось пометить вершину $5$ и $\varepsilon = \varepsilon(5) = 3$.
	
	\bigskip
	
	Используя пометки, восстановим путь с максимальным потоком от вершины $5$ до вершины $1$ (первое значение пометки --- это та вершина, откуда в эту вершину пришли, а знак --- направление дуги). Над дугами запишем значение, на которое нужно увеличить поток через дугу, ($\varepsilon$ или -$\varepsilon$) в зависимости от направления.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {\squared{1}} node[above] {$+3$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{2}} node[above] {$+3$} (3);
			\draw[->, edge] (3) -- node[below] {\squared{6}} node[above] {$+3$} (4);
		\end{tikzpicture}
		\caption{Путь для шага 1}
	\end{figure}
	
	Изменим поток через соответствующие дуги и запишем результат в таблицу (на следующую итерацию).
	
	\begin{table}[H]
		\centering
		\begin{tabular}{ | c | c | c | } \hline
			\diagbox{Вершина}{Итерация} & 1 & 2 \\ \hline
			1 & $(-, \infty)$ & $(-, \infty)$   \\ \hline
			2 & $(+1, 3)$ &  \\ \hline
			3 & $(+1, 5)$ &  \\ \hline
			4 & $(+2, 3)$ &  \\ \hline
			5 & $(+4, 3)$ &  \\ \hline
			{Дуга} & \multicolumn{2}{ c|}{} \\ \hline
			1 & $(3, 0)$ & $(3, 3)$ \\ \hline
			2 & $(6, 0)$ & $(6, 3)$ \\ \hline
			3 & $(5, 0)$ & $(5, 0)$ \\ \hline
			4 & $(2, 0)$ & $(2, 0)$ \\ \hline
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=3$ из вершины $k=1$ по прямой дуге $j=3$:
		\[
		\varepsilon(3) = \min(\infty, 5 - 0)=5 \quad \Longrightarrow \quad (+1, 5)
		\]
		
		\begin{note}
			Вершину $i=2$ нельзя было пометить из вершины $k=1$ по прямой дуге $j=1$, так как $b_1 - x_1 = 3 - 3 \ngtr 0$.
		\end{note}
		
		\item Помечаем вершину $i=4$ из вершины $k=3$ по прямой дуге $j=5$:
		\[
		\varepsilon(4) = \min(5, 2 - 0)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=3$ по прямой дуге $j=4$:
		\[
		\varepsilon(5) = \min(5, 2 - 0)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
	\end{enumerate}
	...
	Нам удалось пометить вершину $5$ и $\varepsilon = \varepsilon(5) = 2$.
		...
		\begin{tikzpicture}[
			...
			\node[vertex] (1) at (0, 0) {$1$};
			\node[vertex] (2) at (1, 0) {$3$};
			\node[vertex] (3) at (2, 0) {$5$};
			
			\draw[->, edge] (1) -- node[below] {\squared{3}} node[above] {$+2$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{4}} node[above] {$+2$} (3);
		\end{tikzpicture}
		\caption{Путь для шага 2}
	\end{figure}
	
	Добавим значения к потокам и запишем результат в таблицу для следующей итерации:
	
	\begin{table}[H]
		\centering
		\begin{tabular}{ | c | c | c | c |} \hline
			\diagbox{Вершина}{Итерация} & 1 & 2 & 3 \\ \hline
			1 & $(-, \infty)$ & $(-, \infty)$ & $(-, \infty)$   \\ \hline
			2 & $(+1, 3)$ & &  \\ \hline
			3 & $(+1, 5)$ & $(+1, 5)$ &  \\ \hline
			4 & $(+2, 3)$ & $(+3, 2)$ &  \\ \hline
			5 & $(+4, 3)$ & $(+3, 2)$ &  \\ \hline
			{Дуга} & \multicolumn{3}{ c|}{} \\ \hline
			1 & $(3, 0)$ & $(3, 3)$ & $(3, 3)$ \\ \hline
			2 & $(6, 0)$ & $(6, 3)$ & $(6, 3)$ \\ \hline
			3 & $(5, 0)$ & $(5, 0)$ & $(5, 2)$ \\ \hline
			4 & $(2, 0)$ & $(2, 0)$ & $(2, 2)$ \\ \hline
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=3$ из вершины $k=1$ по прямой дуге $j=3$:
		\[
		\varepsilon(3) = \min(\infty, 5 - 2)=3 \quad \Longrightarrow \quad (+1, 3)
		\]
		
		\item Помечаем вершину $i=4$ из вершины $k=3$ по прямой дуге $j=5$:
		\[
		\varepsilon(4) = \min(3, 2 - 0)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=4$ по прямой дуге $j=6$:
		\[
		\varepsilon(5) = \min(2, 4 - 3)=1 \quad \Longrightarrow \quad (+4, 1)
		\]
	\end{enumerate}
	...
	Нам удалось пометить вершину $5$ и $\varepsilon = \varepsilon(5) = 1$.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {\squared{3}} node[above] {$+1$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{5}} node[above] {$+1$} (3);
			\draw[->, edge] (3) -- node[below] {\squared{6}} node[above] {$+1$} (4);
		\end{tikzpicture}
		\caption{Путь для шага 3}
	\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | c | c | c | c | c | } \hline
			\diagbox{Вершина}{Итерация} & 1 & 2 & 3 & 4 \\ \hline
			1 & $(-, \infty)$ & $(-, \infty)$ & $(-, \infty)$ & $(-, \infty)$ \\ \hline
			2 & $(+1, 3)$ & & &  \\ \hline
			3 & $(+1, 5)$ & $(+1, 5)$ & $(+1, 3)$ & \\ \hline
			4 & $(+2, 3)$ & $(+3, 2)$ & $(+3, 2)$ & \\ \hline
			5 & $(+4, 3)$ & $(+3, 2)$ & $(+4, 1)$ & \\ \hline
			{Дуга} & \multicolumn{4}{ c|}{} \\ \hline
			1 & $(3, 0)$ & $(3, 3)$ & $(3, 3)$ & $(3, 3)$ \\ \hline
			2 & $(6, 0)$ & $(6, 3)$ & $(6, 3)$ & $(6, 3)$ \\ \hline
			3 & $(5, 0)$ & $(5, 0)$ & $(5, 2)$ & $(5, 3)$ \\ \hline
			4 & $(2, 0)$ & $(2, 0)$ & $(2, 2)$ & $(2, 2)$ \\ \hline
			5 & $(2, 0)$ & $(2, 0)$ & $(2, 0)$ & $(2, 1)$ \\ \hline
			6 & $(4, 0)$ & $(4, 3)$ & $(4, 3)$ & $(4, 4)$ \\ \hline
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=3$ из вершины $k=1$ по прямой дуге $j=3$:
		\[
		\varepsilon(3) = \min(\infty, 5 - 3)=2 \quad \Longrightarrow \quad (+1, 2)
		\]
		...
		\item Помечаем вершину $i=4$ из вершины $k=3$ по прямой дуге $j=5$:
		\[
		\varepsilon(4) = \min(2, 2 - 1)=1 \quad \Longrightarrow \quad (+3, 1)
		\]
		
		\item Помечаем вершину $i=2$ из вершины $k=4$ по обратной дуге $j=2$:
		\[
		\varepsilon(2) = \min(1, 3)=1 \quad \Longrightarrow \quad (-4, 1)
		\]
		
		\begin{note}
			Мы пометили все вершины, кроме $i=5$, а пометить её нельзя ни из вершины $k=3$, ни из вершины $k=4$. Поэтому завершаем итерацию и алгоритм.
		\end{note}
	\end{enumerate}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | c | c | c | c | c | } \hline
			...
			1 & $(-, \infty)$ & $(-, \infty)$ & $(-, \infty)$ & $(-, \infty)$ \\ \hline
			2 & $(+1, 3)$ & & & $(-4, 1)$  \\ \hline
			3 & $(+1, 5)$ & $(+1, 5)$ & $(+1, 3)$ & $(+1, 2)$ \\ \hline
			4 & $(+2, 3)$ & $(+3, 2)$ & $(+3, 2)$ & $(+3, 1)$ \\ \hline
			5 & $(+4, 3)$ & $(+3, 2)$ & $(+4, 1)$ & \\ \hline
			{Дуга} & \multicolumn{4}{ c|}{} \\ \hline
			1 & $(3, 0)$ & $(3, 3)$ & $(3, 3)$ & $(3, 3)$ \\ \hline
			2 & $(6, 0)$ & $(6, 3)$ & $(6, 3)$ & $(6, 3)$ \\ \hline
	...
	Нам не удалось пометить вершину 5. Алгоритм завершён.
\end{itemize}

\bigskip

\textbf{Построение разреза}

Используя заполненную таблицу, построим разрез $(S, \bar{S})$ следующим образом: 
\[
S = \big\{1, 2, 3, 4\big\}, \quad \bar{S} = \big\{5\big\},
\]

то есть $S$ --- помеченные на последней итерации вершины, $\bar{S}$ --- не помеченные.

Вычислим пропускную способность разреза:
\[
\boxed{B(S, \bar{S}) = b_{(3,5)} + b_{(4,5)} = 2 + 4 = 6}.
\]

Вычислим итоговую величину потока $v(x)$ через <<баланс>> вершины $1$ на последней итерации:
\[
\boxed{v(x) = x_{(1,3)} + x_{(1,2)} = 3 + 3 = 6}.
\]

\begin{note}
	Значение $v(x)$ также можно было вычислить через <<баланс>> вершины $5$ на последней итерации:
	\[
	v(x) = x_{(3,5)} + x_{(4,5)} = 2 + 4 = 6.
	\]
	
	Также $v(x)$ можно вычислить как сумму $\varepsilon$ по всем итерациям:
	\[
	v(x) = 3 + 2 + 1 = 6.
	\]
\end{note}

Видно, что $v(x) = B(S, \bar{S})$, поэтому полученный поток действительно является максимальным.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\draw[->, edge] (1) -- node[above=2mm, left=1mm] {\squared{1}} node[below=1mm, right=1mm] {$7$} (2);
		\draw[->, edge] (2) -- node[above] {\squared{2}} node[below] {$6$} (3);
		\draw[->, edge] (3) -- node[above right] {\squared{3}} node[below left] {$4$} (6);
		
		\draw[->, edge] (1) -- node[below left] {\squared{4}} node[above right] {$6$} (4);
		\draw[->, edge] (4) -- node[below] {\squared{5}} node[above] {$2$} (5);
		\draw[->, edge] (5) -- node[below right] {\squared{6}} node[above left] {$1$} (6);
		
		\draw[->, edge] (3) -- node[right] {\squared{7}} node[left] {$3$} (5);
		\draw[->, edge] (4) -- node[below=1mm, right=3mm] {\squared{8}} node[above=6mm, right=3mm] {$9$} (6);
		\draw[->, edge] (5) -- node[above=2mm, left=3mm] {\squared{9}} node[below=2mm, right=3mm] {$3$} (2);
	\end{tikzpicture}
	\caption{Сеть}
\end{figure}
...
Отдельно таблицу для вершин вести не будем: для каждой итерации в конце процесса пометки будет приведена табличка с метками для текущей итерации.
...
\begin{itemize}[nosep]
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=4$ из вершины $k=1$ по прямой дуге $j=4$:
		\[
		\varepsilon(4) = \min(\infty, 6 - 0)=5 \quad \Longrightarrow \quad (+1, 6)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=4$ по прямой дуге $j=5$:
		\[
		\varepsilon(5) = \min(6, 2 - 0)=2 \quad \Longrightarrow \quad (+4, 2)
		\]
		
		\item Помечаем вершину $i=2$ из вершины $k=5$ по прямой дуге $j=9$:
		\[
		\varepsilon(2) = \min(2, 3 - 0)=2 \quad \Longrightarrow \quad (+5, 2)
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=2$ по прямой дуге $j=2$:
		\[
		\varepsilon(3) = \min(2, 6 - 0)=2 \quad \Longrightarrow \quad (+2, 2)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=3$ по прямой дуге $j=3$:
		\[
		\varepsilon(6) = \min(2, 4 - 0)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
	\end{enumerate}
	...
	Нам удалось пометить вершину $6$ и $\varepsilon = \varepsilon(6) = 2$.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			\node[vertex] (1) at (0, 0) {$1$};
			\node[vertex] (2) at (1, 0) {$4$};
			\node[vertex] (3) at (2, 0) {$5$};
			\node[vertex] (4) at (3, 0) {$2$};
			\node[vertex] (5) at (4, 0) {$3$};
			\node[vertex] (6) at (5, 0) {$6$};
			
			\draw[->, edge] (1) -- node[below] {\squared{4}} node[above] {$+2$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{5}} node[above] {$+2$} (3);
			\draw[->, edge] (3) -- node[below] {\squared{9}} node[above] {$+2$} (4);
			\draw[->, edge] (4) -- node[below] {\squared{6}} node[above] {$+2$} (5);
			\draw[->, edge] (5) -- node[below] {\squared{4}} node[above] {$+2$} (6);
		\end{tikzpicture}
		\caption{Путь для шага 1}
	\end{figure}
	...
	\begin{table}[H]
		\centering
		\begin{tabular}{ | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ \\ \hline
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=2$ из вершины $k=1$ по прямой дуге $j=1$:
		\[
		\varepsilon(2) = \min(\infty, 7 - 0)=7 \quad \Longrightarrow \quad (+1, 7)
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=2$ по прямой дуге $j=2$:
		\[
		\varepsilon(3) = \min(7, 6 - 2)=4 \quad \Longrightarrow \quad (+2, 4)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=3$ по прямой дуге $j=3$:
		\[
		\varepsilon(6) = \min(4, 4 - 2)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {\squared{1}} node[above] {$+2$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{2}} node[above] {$+2$} (3);
			\draw[->, edge] (3) -- node[below] {\squared{3}} node[above] {$+2$} (4);
		\end{tikzpicture}
		\caption{Путь для шага 2}
	\end{figure}
	...
	\begin{table}[H]
		\centering
		\begin{tabular}{ | c | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 & 3 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ & $(7, 2)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ & $(6, 4)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ & $(4, 4)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ & $(6, 2)$ \\ \hline
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=2$ из вершины $k=1$ по прямой дуге $j=1$:
		\[
		\varepsilon(2) = \min(\infty, 7 - 2)=5 \quad \Longrightarrow \quad (+1, 5)
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=2$ по прямой дуге $j=2$:
		\[
		\varepsilon(3) = \min(5, 6 - 2)=2 \quad \Longrightarrow \quad (+2, 2)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=3$ по прямой дуге $j=7$:
		\[
		\varepsilon(5) = \min(2, 3 - 0)=2 \quad \Longrightarrow \quad (+3, 2)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=5$ по прямой дуге $j=6$:
		\[
		\varepsilon(6) = \min(2, 1 - 0)=1 \quad \Longrightarrow \quad (+5, 1)
		\]
	...
	Нам удалось пометить вершину $6$ и $\varepsilon = \varepsilon(6) = 1$.
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {\squared{1}} node[above] {$+1$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{2}} node[above] {$+1$} (3);
			\draw[->, edge] (3) -- node[below] {\squared{7}} node[above] {$+1$} (4);
			\draw[->, edge] (4) -- node[below] {\squared{6}} node[above] {$+1$} (5);
		\end{tikzpicture}
		\caption{Путь для шага 3}
	\end{figure}
	...
	\begin{table}[H]
		\centering
		\begin{tabular}{ | c | c | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 & 3 & 4 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ & $(7, 2)$ & $(7, 3)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ & $(6, 4)$ & $(6, 5)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ & $(4, 4)$ & $(4, 4)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ \\ \hline
			5 & $(2, 0)$ & $(2, 2)$ & $(2, 2)$ & $(2, 2)$ \\ \hline
			6 & $(1, 0)$ & $(1, 0)$ & $(1, 0)$ & $(1, 1)$ \\ \hline
			7 & $(3, 0)$ & $(3, 0)$ & $(3, 0)$ & $(3, 1)$ \\ \hline
			8 & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ \\ \hline
			9 & $(3, 0)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ \\ \hline
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=2$ из вершины $k=1$ по прямой дуге $j=1$:
		\[
		\varepsilon(2) = \min(\infty, 7 - 3)=4 \quad \Longrightarrow \quad (+1, 4)
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=2$ по прямой дуге $j=2$:
		\[
		\varepsilon(3) = \min(4, 6 - 5)=1 \quad \Longrightarrow \quad (+2, 1)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=3$ по прямой дуге $j=7$:
		\[
		\varepsilon(5) = \min(1, 3 - 1)=1 \quad \Longrightarrow \quad (+3, 1)
		\]
		
		\item Помечаем вершину $i=4$ из вершины $k=5$ по обратной дуге $j=5$:
		\[
		\varepsilon(4) = \min(1, 2)=1 \quad \Longrightarrow \quad (-5, 1)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=4$ по прямой дуге $j=8$:
		\[
		\varepsilon(6) = \min(1, 9 - 0)=1 \quad \Longrightarrow \quad (+4, 1)
		\]
	...
	\begin{tikzpicture}[
		...
		\draw[->, edge] (2) -- node[below] {\squared{2}} node[above] {$+1$} (3);
		\draw[->, edge] (3) -- node[below] {\squared{7}} node[above] {$+1$} (4);
		\draw[->, edge] (5) -- node[below] {\squared{5}} node[above] {$-1$} (4);
		\draw[->, edge] (5) -- node[below] {\squared{8}} node[above] {$+1$} (6);
	\end{tikzpicture}
	\caption{Путь для шага 4}
\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | c | c | c | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 & 3 & 4 & 5 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ & $(7, 2)$ & $(7, 3)$ & $(7, 4)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ & $(6, 4)$ & $(6, 5)$ & $(6, 6)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ \\ \hline
			5 & $(2, 0)$ & $(2, 2)$ & $(2, 2)$ & $(2, 2)$ & $(2, 1)$ \\ \hline
			6 & $(1, 0)$ & $(1, 0)$ & $(1, 0)$ & $(1, 1)$ & $(1, 1)$ \\ \hline
			7 & $(3, 0)$ & $(3, 0)$ & $(3, 0)$ & $(3, 1)$ & $(3, 2)$ \\ \hline
			8 & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 1)$ \\ \hline
			9 & $(3, 0)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ \\ \hline
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=4$ из вершины $k=1$ по прямой дуге $j=4$:
		\[
		\varepsilon(4) = \min(\infty, 6 - 2)=4 \quad \Longrightarrow \quad (+1, 4)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=4$ по прямой дуге $j=8$:
		\[
		\varepsilon(6) = \min(4, 9 - 1)=4 \quad \Longrightarrow \quad (+4, 4)
		\]
	...
	Нам удалось пометить вершину $6$ и $\varepsilon = \varepsilon(6) = 4$.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			\node[vertex] (1) at (0, 0) {$1$};
			\node[vertex] (2) at (1, 0) {$4$};
			\node[vertex] (3) at (2, 0) {$6$};
			
			\draw[->, edge] (1) -- node[below] {\squared{4}} node[above] {$+4$} (2);
			\draw[->, edge] (2) -- node[below] {\squared{8}} node[above] {$+4$} (3);
		\end{tikzpicture}
		\caption{Путь для шага 5}
	\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | c | c | c | c | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 & 3 & 4 & 5 & 6 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ & $(7, 2)$ & $(7, 3)$ & $(7, 4)$ & $(7, 4)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ & $(6, 4)$ & $(6, 5)$ & $(6, 6)$ & $(6, 6)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ & $(6, 6)$ \\ \hline
			5 & $(2, 0)$ & $(2, 2)$ & $(2, 2)$ & $(2, 2)$ & $(2, 1)$ & $(2, 1)$ \\ \hline
			6 & $(1, 0)$ & $(1, 0)$ & $(1, 0)$ & $(1, 1)$ & $(1, 1)$ & $(1, 1)$ \\ \hline
			7 & $(3, 0)$ & $(3, 0)$ & $(3, 0)$ & $(3, 1)$ & $(3, 2)$ & $(3, 2)$ \\ \hline
			8 & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 1)$ & $(9, 5)$ \\ \hline
			9 & $(3, 0)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ \\ \hline
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=2$ из вершины $k=1$ по прямой дуге $j=1$:
		\[
		\varepsilon(2) = \min(\infty, 7 - 5)=2 \quad \Longrightarrow \quad (+1, 2)
		\]
		
		\item Помечаем вершину $i=5$ из вершины $k=2$ по обратной дуге $j=9$:
		\[
		\varepsilon(5) = \min(2, 2)=2 \quad \Longrightarrow \quad (-2, 2)
		\]
		
		\item Помечаем вершину $i=4$ из вершины $k=5$ по обратной дуге $j=5$:
		\[
		\varepsilon(4) = \min(2, 1)=1 \quad \Longrightarrow \quad (-5, 1)
		\]
		
		\item Помечаем вершину $i=6$ из вершины $k=4$ по прямой дуге $j=8$:
		\[
		\varepsilon(6) = \min(1, 9 - 5)=1 \quad \Longrightarrow \quad (+4, 1)
		\]
	...
	Нам удалось пометить вершину $6$ и $\varepsilon = \varepsilon(6) = 1$.
		...
		\begin{tikzpicture}[
			...
			\draw[->, edge] (1) -- node[below] {\squared{1}} node[above] {$+1$} (2);
			\draw[->, edge] (3) -- node[below] {\squared{9}} node[above] {$-1$} (2);
			\draw[->, edge] (4) -- node[below] {\squared{5}} node[above] {$-1$} (3);
			\draw[->, edge] (4) -- node[below] {\squared{8}} node[above] {$+1$} (5);
		\end{tikzpicture}
		\caption{Путь для шага 6}
	\end{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ | c | c | c | c | c | c | c | c | } \hline
			\diagbox{Дуга}{Итерация} & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\ \hline
			1 & $(7, 0)$ & $(7, 0)$ & $(7, 2)$ & $(7, 3)$ & $(7, 4)$ & $(7, 4)$ & $(7, 5)$ \\ \hline
			2 & $(6, 0)$ & $(6, 2)$ & $(6, 4)$ & $(6, 5)$ & $(6, 6)$ & $(6, 6)$ & $(6, 6)$ \\ \hline
			3 & $(4, 0)$ & $(4, 2)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ & $(4, 4)$ \\ \hline
			4 & $(6, 0)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ & $(6, 2)$ & $(6, 6)$ & $(6, 6)$ \\ \hline
			5 & $(2, 0)$ & $(2, 2)$ & $(2, 2)$ & $(2, 2)$ & $(2, 1)$ & $(2, 1)$ & $(2, 0)$ \\ \hline
			6 & $(1, 0)$ & $(1, 0)$ & $(1, 0)$ & $(1, 1)$ & $(1, 1)$ & $(1, 1)$ & $(1, 1)$ \\ \hline
			7 & $(3, 0)$ & $(3, 0)$ & $(3, 0)$ & $(3, 1)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ \\ \hline
			8 & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 0)$ & $(9, 1)$ & $(9, 5)$ & $(9, 6)$ \\ \hline
			9 & $(3, 0)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 2)$ & $(3, 1)$ \\ \hline
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[nosep]
		...
		\item Помечаем вершину $i=5$ из вершины $k=2$ по обратной дуге $j=9$:
		\[
		\varepsilon(5) = \min(2, 1)=1 \quad \Longrightarrow \quad (-2, 1)
		\]
		
		\item Помечаем вершину $i=3$ из вершины $k=5$ по обратной дуге $j=7$:
		\[
		\varepsilon(3) = \min(1, 2)=1 \quad \Longrightarrow \quad (-5, 1)
		\]
	...
	Нам не удалось пометить вершину $6$. Итерации окончены.
\end{itemize}
...
Используя заполненную таблицу, построим разрез $(S, \bar{S})$ следующим образом: 
\[
S = \big\{1, 2, 3, 5\big\}, \quad \bar{S} = \big\{4, 6\big\},
\]
...
Вычислим пропускную способность разреза:
\[
\boxed{B(S, \bar{S}) = b_{(3,6)} + b_{(5,6)} + b_{(1,4)} = 4 + 1 + 6 = 11}.
\]

Вычислим итоговую величину потока $v(x)$ через <<баланс>> вершины $1$ на последней итерации:
\[
\boxed{v(x) = x_{(1,2)} + x_{(1,4)} = 5 + 6 = 11}.
\]
